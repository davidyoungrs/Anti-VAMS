...
10. **Session Management (Inactivity Timeout)** (Priority: Medium)
    - **Why**: ISO 27002 8.17 (Clock Synchronization) & Session control.
    - **Task**: Implement client-side inactivity timer to auto-logout users after X minutes of idle time.
...
13. **Incident Response Playbook Integration** (Priority: Medium)
    - **Why**: ISO 27032 13.3 (Incident Management).
    - **Task**: Create an admin-accessible "Emergency Mode" feature (e.g., global "ReadOnly" switch) to contain active threats during a breach.
...
21. **NTP Time Synchronization Assurance** (Priority: Low)
    - **Why**: QCB/NCA mandate accurate timestamping for forensic validity.
    - **Task**: Verify cloud server clock sources and expose "Last Sync Status" in the Admin Panel.
