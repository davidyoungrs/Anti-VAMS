...
export default function InspectionFormButterflyValve({ valveId, inspectionId, onBack, onSave }) {
    ...
    const loadInspection = async () => {
        ...
        if (existing) setInspection(existing);
    };
    ...
    const componentGroups = VALVE_COMPONENT_CONFIGS['Butterfly Valve'];

    return (
        <div className="inspection-form">
            <div className="inspection-header">
                <button onClick={onBack} className="back-button">← Back to Inspections</button>
                <h2>{inspectionId ? 'Edit Butterfly Valve Inspection' : 'New Butterfly Valve Inspection'}</h2>
            </div>
            <ValveDataHeader valveData={valveData} />
            <form onSubmit={handleSubmit}>
                <InspectionGeneralInfo inspection={inspection} onChange={handleFieldChange} />
                <section className="form-section">
                    <h3>Component Inspection</h3>
                    <p className="section-description">Inspect each component and record condition, action taken, and details.</p>
                    {Object.entries(componentGroups).map(([groupName, components]) => (
                        <div key={groupName} className="component-group">
                            <div className="group-header" onClick={() => toggleSection(groupName)}>
                                <h4>{groupName}</h4>
                                <span className="toggle-icon">{expandedSections[groupName] ? '▼' : '▶'}</span>
                            </div>
                            {expandedSections[groupName] && (
                                <div className="component-list">
                                    {components.map(componentKey => (
                                        <div key={componentKey} className="component-item">
                                            <div className="component-name">{COMPONENT_LABELS[componentKey] || componentKey}</div>
                                            <div className="component-fields">
                                                ...
                                                <div className="field-group">
                                                    ...
                                                    <input type="text" value={inspection.components[componentKey]?.partNumber || ''} onChange={(e) => handleComponentChange(componentKey, 'partNumber', e.target.value)} placeholder="Part #" />
                                                </div>
                                                <div className="field-group">
                                                    <label>Material</label>
                                                    <input type="text" value={inspection.components[componentKey]?.material || ''} onChange={(e) => handleComponentChange(componentKey, 'material', e.target.value)} placeholder="Material" />
                                                </div>
                                                <div className="field-group full-width">
                                                    <label>Comments</label>
                                                    <input type="text" value={inspection.components[componentKey]?.comments || ''} onChange={(e) => handleComponentChange(componentKey, 'comments', e.target.value)} placeholder="Additional notes" />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                ...
                <section className="form-section">
                    <h3>Repair Notes</h3>
                    <textarea value={inspection.repairNotes || ''} onChange={(e) => handleFieldChange('repairNotes', e.target.value)} placeholder="Enter detailed repair notes..." rows="6" />
                </section>
                <InspectionPhotos photoFiles={photoFiles} existingPhotos={inspection.inspectionPhotos} onAddPhotos={handlePhotoChange} onRemoveNewPhoto={removePhoto} />
                <div className="form-actions">
                    <button type="button" onClick={onBack} className="cancel-button">Cancel</button>
                    <button type="submit" disabled={isSaving} className="save-button">{isSaving ? 'Saving...' : 'Save Inspection'}</button>
                </div>
            </form>
        </div>
    );
